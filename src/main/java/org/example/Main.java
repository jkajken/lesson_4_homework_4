...
public class Main {
    ...
    public static void medicalHealing() {
        for (int i = 0; i < heroesHealth.length; i++) {
            if (heroesHealth[i] != heroesHealth[3] && heroesHealth[3] > 0 && heroesHealth[i] <= 100) {
                Random random = new Random();
                int healingScore = random.nextInt(50);
                heroesHealth[i] += healingScore;
                System.out.println("Medical help " + heroesAttackType[i] + " " + healingScore);
                break;
            }
        }
    }
